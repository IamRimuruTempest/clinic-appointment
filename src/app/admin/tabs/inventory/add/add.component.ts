  ...
  async onSubmit(values: Inventory) {
    ...
    try {
      ...
      const newHistory: InventoryHistory = {
        ...
        ending_quantity:
          Number(values.quantity) + Number(this.inventory!.quantity),
      };
      await this.dataService.addInventoryHistory(
        this.inventory!.id!,
        newHistory
      );
      successMsg = 'Successfully added stock.';
      errorMsg = 'Add stock failed.';
      loading.dismiss();
